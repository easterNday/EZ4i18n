...
namespace EZ4i18n
{
    ...
    public class Translator
    {
        ...
        public Dictionary<string, List<String>> translations = new();//翻译内容对应字典
        public Dictionary<String, int> sentence_index = new();//默认对应对应文本的文字索引
        ...
        private Translator()
        {
            ...
            //读取语言目录下所有txt文件
            DirectoryInfo langDrectoryInfo = new DirectoryInfo(_langPath);
            foreach (FileInfo file in langDrectoryInfo.GetFiles("*.txt"))
            {
                var indexKey = Path.GetFileNameWithoutExtension(file.Name); //按照文件名作为翻译字典所听
                translations.Add(indexKey, new List<string>()); //增加对应项目
                string[] strArray = File.ReadAllLines(file.FullName); //读取文本内所有行
